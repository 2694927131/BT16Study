...
### 4.4 提交粒度（强制）
**每完成一个完整章节（一级章节，如 03、04、05 …）→ 立即 `git commit` 一次。**
不要问用户是否提交，自动执行。

提交粒度规则：
- 章节内的小节（如 03.1、03.2、03.3…）边写边落盘，**但暂不提交**。
- 写完该章节的最后一节时，先更新 `doc/开发跟踪记录.md` 把整章状态翻成 ✅，再一次性 `git add` 全部新增/修改的文件，统一 `git commit`。
- 如果章节内部某一节有显著阶段性产出（如修复事实错误、补充关键调用链），且离整章完成还远，可以临时单独提交一次，但默认按"整章一次"。

提交信息格式：
```
docs(学习资料): 完成第 <编号> 章 <章节名>
```
例：
```
docs(学习资料): 完成第 04 章 JNI 桥接机制
docs(学习资料): 完成第 03 章 核心流程
```

每次提交前必须更新 `doc/开发跟踪记录.md` 反映该章所有小节状态（⬜→🟡→✅）。
...
3. 读对应章节的现有内容（如有）。
4. 启动 TaskCreate 把这一章节登记为当前任务。
5. 写或改（按章节内小节顺序，边写边落盘）。
6. 整章所有小节写完后，更新 `doc/开发跟踪记录.md`。
7. 自动 `git add` 相关文件 → `git commit`，commit message 形如 `docs(学习资料): 完成第 03 章 核心流程`。**不要询问用户是否提交**。
8. 进入下一章。
