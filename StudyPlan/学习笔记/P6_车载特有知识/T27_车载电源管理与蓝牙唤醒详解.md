...
inline bool is_legal_power_mode(tBTM_PM_MODE mode) {
  switch (mode & ~BTM_PM_MD_FORCE) {  // 💡 位运算: 去掉FORCE位, 只看基础模式
...
void btsnd_hcic_sniff_mode(uint16_t handle, uint16_t max_sniff_period,
  ...
  uint8_t* pp = (uint8_t*)(p + 1);    // 💡 指针算术: p+1跳过BT_HDR头, 指向数据区
                                        //         Java无指针算术, 用ByteBuffer.position()

  p->len = HCIC_PREAMBLE_SIZE + HCIC_PARAM_SIZE_SNIFF_MODE;  // 💡 命令总长度
  p->offset = 0;

  // 💡 C++: 宏UINT16_TO_STREAM将uint16_t按小端序写入字节流
  //         Java中用ByteBuffer.putShort() + order(ByteOrder.LITTLE_ENDIAN)
  UINT16_TO_STREAM(pp, HCI_SNIFF_MODE);   // 💡 HCI Opcode: Sniff Mode命令
...
tBTM_STATUS BTM_SetPowerMode(uint8_t pm_id, const RawAddress& remote_bda,
  ...
  if (p_cb == nullptr) {                // 💡 设备未连接, 无法设置电源模式
...
typedef uint8_t tBTM_PM_MODE;  // 💡 typedef创建类型别名
```
...
- [x] 10个标准章节全部包含
- [x] Mermaid图 ≥ 5个（状态图2 + 架构图2 + 时序图1 + 流程图2 = 7个）
- [x] 代码导航表 ≥ 8条（12条）
- [x] 代码片段 ≥ 7个（7个），每个都有逐行💡注释
- [x] 每个代码片段标注了源文件路径和行号
- [x] C++知识卡片3个，均有Java对比
- [x] Java ↔ C++对照表完整（14项）
- [x] 问题排查SOP含Mermaid流程图
- [x] 动手练习分入门/进阶/实战三级
- [x] 关键源码索引表完整（20条）
- [x] 章节标题仅使用标准emoji（📋🗺️🔍📖💡🗂️🐛🛠️📚）
- [x] 全文中文撰写
